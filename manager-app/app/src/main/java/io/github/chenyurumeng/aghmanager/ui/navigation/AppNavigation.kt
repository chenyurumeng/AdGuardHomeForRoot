    ...
    Scaffold(
        ...
        NavHost(navController = navController, startDestination = "home") {
            ...
            composable(
                ...
                val vm: AghProtectionViewModel = viewModel(
                    factory = AghProtectionViewModel.Factory(
                        instance = instance,
                        repository = aghProtectionRepository
                    )
                )
                LaunchedEffect(vm) {
                    vm.messages.collect { snackbarHostState.showSnackbar(it) }
            ...
            composable(
                ...
                val vm: AghStatisticsViewModel = viewModel(
                    factory = AghStatisticsViewModel.Factory(
                        instance = instance,
                        repository = aghStatisticsRepository
                    )
                )
                LaunchedEffect(vm) {
                    vm.messages.collect { snackbarHostState.showSnackbar(it) }
