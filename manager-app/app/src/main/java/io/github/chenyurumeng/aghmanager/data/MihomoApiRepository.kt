...
class MihomoApiRepository {
    ...
    private suspend fun loadProcessMetrics(): MihomoProcessMetrics {
        ...
        result.stdout.lineSequence().forEach { line ->
            when {
                line.startsWith("RSS=") ->
                    rss = line.substringAfter('=').trim().toLongOrNull() ?: 0L
                line.startsWith("SWAP=") ->
                    swap = line.substringAfter('=').trim().toLongOrNull() ?: 0L
                line.startsWith("CPU=") ->
                    cpu = line.substringAfter('=').trim().toDoubleOrNull() ?: 0.0
            }
        }

        return MihomoProcessMetrics(
            rssKb = rss,
            swapKb = swap,
            cpuPercent = cpu
        )
    ...
    private fun loadGroups(endpoint: Endpoint): Result<List<MihomoGroup>> {
        ...
        return runCatching {
            ...
            groups.filter { !it.hidden }.sortedBy { it.name }
        }
    }
    ...
    private fun normalizeController(raw: String): String {
        ...
        return when {
            ...
            value.startsWith("0.0.0.0:") ->
                "http://127.0.0.1:" + value.substringAfter(':')
            value.startsWith("*:") ->
                "http://127.0.0.1:" + value.substringAfter(':')
            value.startsWith("[::]:") ->
                "http://127.0.0.1:" + value.substringAfterLast(':')
            value.startsWith(":") ->
